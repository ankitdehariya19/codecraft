...
const Dashboards = () => {
  ...
  return (
    <div className='w-80 h-screen transition-opacity duration-1000 ease-in-out'>
              ...
              {activeMenu === menuItem.title.toLowerCase() && menuItem.submenus && (
                <ul className='ml-6 mt-2'>
                  {menuItem.submenus.map((submenu, index) => (
                    <li
                      key={index}
                      className='mb-4 text-sm font-medium text text-slate-400 hover:text-stone-600 flex items-center transition-opacity duration-1000 ease-in-out'
                    >
                      <span className='mr-2'>
